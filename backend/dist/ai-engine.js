 ...
 */
async function analyzeDispute(projectId, milestoneId, initiator, contractDetails = 'Standard dispute analysis') {
    // Rule-based analysis
...
function calculateComplianceScore(details) {
    let score = 70;
    if (details.includes('complete'))
        score += 10;
    if (details.includes('approved'))
        score += 10;
    if (details.includes('verified'))
        score += 10;
    if (details.includes('incomplete'))
        score -= 20;
...
function calculateQualityScore(details) {
    let score = 65;
    if (details.includes('high quality'))
        score += 20;
    if (details.includes('excellent'))
        score += 15;
    if (details.includes('professional'))
        score += 10;
    if (details.includes('poor quality'))
        score -= 25;
    if (details.includes('bugs'))
        score -= 15;
    return Math.max(0, Math.min(100, score));
}
function calculateTimelineScore(details) {
    let score = 75;
    if (details.includes('on time'))
        score += 15;
    if (details.includes('early'))
        score += 20;
    if (details.includes('delayed'))
        score -= 25;
    if (details.includes('late'))
        score -= 20;
    return Math.max(0, Math.min(100, score));
}
