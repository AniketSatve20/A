...
app.get('/api/stats', (req, res) => {
    try {
        const stats = (0, database_1.getProjectStats)();
...
app.get('/api/disputes', (req, res) => {
    try {
        ...
        const disputes = (0, database_1.getDisputeHistory)(limit);
...
app.get('/api/users/:address/reputation', (req, res) => {
    try {
        ...
        const user = (0, database_1.getOrCreateUser)(address);
...
app.get('/api/users/:address/profile', auth_1.authMiddleware, (req, res) => {
    try {
        const { address } = req.params;
        const user = (0, database_1.getOrCreateUser)(address);
        res.json(user);
...
app.get('/api/projects', (req, res) => {
    try {
        ...
        logger_1.logger.info(`Retrieved ${projects.length} projects`);
        res.json(projects);
    }
    catch (error) {
...
app.post('/api/projects', auth_1.authMiddleware, (req, res) => {
    try {
        ...
        logger_1.logger.info(`Project created: ${projectId}`);
...
app.post('/api/disputes/analyze', auth_1.authMiddleware, (req, res) => {
    try {
        ...
        // Run AI analysis
        (0, ai_engine_1.analyzeDispute)(projectId, milestoneId, initiator, details)
            .then(analysis => {
            logger_1.logger.info(`Dispute analyzed: ${projectId}/${milestoneId}`);
            res.json(analysis);
        })
            .catch(err => {
            logger_1.logger.error('AI analysis failed', err);
            res.status(500).json({ error: 'Analysis failed' });
        });
    }
    catch (error) {
        logger_1.logger.error('Failed to analyze dispute', error);
        res.status(500).json({ error: error.message });
    }
});
// Record dispute
app.post('/api/disputes', auth_1.authMiddleware, (req, res) => {
    try {
        const { disputeId, projectId, milestoneId, initiator, verdict, confidence } = req.body;
        if (!disputeId || !projectId || !milestoneId) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        (0, database_1.recordDispute)({
            disputeId,
            projectId,
            milestoneId,
            initiator,
        ...
        logger_1.logger.info(`Dispute recorded: ${disputeId}`);
        res.json({ success: true, disputeId });
    }
    catch (error) {
        logger_1.logger.error('Failed to record dispute', error);
        res.status(500).json({ error: error.message });
    }
});
// Update dispute with jury verdict
app.put('/api/disputes/:disputeId/verdict', auth_1.authMiddleware, (req, res) => {
    try {
        ...
        (0, database_1.updateDisputeVerdict)(parseInt(disputeId), verdict, votesFor || 0, votesAgainst || 0);
        logger_1.logger.info(`Dispute verdict recorded: ${disputeId}`);
        res.json({ success: true, disputeId });
    }
    catch (error) {
        logger_1.logger.error('Failed to update dispute', error);
        res.status(500).json({ error: error.message });
    }
});
// Get user disputes
app.get('/api/users/:address/disputes', auth_1.authMiddleware, (req, res) => {
    try {
        const { address } = req.params;
        const limit = parseInt(req.query.limit) || 20;
        const allDisputes = (0, database_1.getDisputeHistory)(limit);
        const disputes = allDisputes.filter((d) => d.initiator_address?.toLowerCase() === address.toLowerCase());
        res.json(disputes);
    }
    catch (error) {
        logger_1.logger.error('Failed to get user disputes', error);
        res.status(500).json({ error: error.message });
    }
});
// Get user projects
app.get('/api/users/:address/projects', auth_1.authMiddleware, (req, res) => {
    try {
        const { address } = req.params;
        ...
        logger_1.logger.error('Failed to get user projects', error);
        res.status(500).json({ error: error.message });
    }
});
// ============ EMAIL OTP ENDPOINTS ============
...
app.post('/api/auth/email/verify-otp', (req, res) => {
    try {
        ...
        logger_1.logger.info(`Email verified: ${email}`);
        res.json({
            success: true,
