...
async function createProviderWithRetry() {
    ...
    for (let attempt = 1; attempt <= RPC_MAX_RETRIES; attempt++) {
        try {
            logger_1.logger.info(`📡 Connecting to RPC (Attempt ${attempt}/${RPC_MAX_RETRIES})...`);
            const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
            await provider.getNetwork();
            logger_1.logger.info('✅ Connected to RPC successfully');
            return provider;
        }
        catch (error) {
            lastError = error;
            logger_1.logger.warn(`⚠️  Connection attempt ${attempt} failed: ${lastError.message}`);
...
async function startListeners() {
    ...
    logger_1.logger.info('🗄️ Database initialized');
    let provider;
    try {
        provider = await createProviderWithRetry();
    }
    catch (error) {
        logger_1.logger.error('❌ Error connecting to RPC:', error);
        process.exit(1);
    }
    let workerAddress;
    try {
        workerAddress = ethers_1.ethers.computeAddress(formattedPrivateKey);
    }
    catch (error) {
        logger_1.logger.error('❌ Error computing address from private key:', error);
        process.exit(1);
    }
    ...
    try {
        ...
        logger_1.logger.info(`📍 Starting from block: ${currentBlock - 5}`);
    }
    catch (error) {
        logger_1.logger.error('❌ Error getting current block:', error);
        process.exit(1);
    }
            ...
            try {
                ...
                if (currentBlock > listener.lastBlock) {
                    try {
                        ...
                        if (logs.length > 0) {
                            logger_1.logger.info(`📊 [${listener.name}] Found ${logs.length} logs in blocks ${listener.lastBlock + 1}-${currentBlock}`);
                            const iface = new ethers_1.ethers.Interface(listener.abi);
                            let parsedCount = 0;
                            logs.forEach(log => {
                                try {
                            ...
                            if (parsedCount > 0) {
                                logger_1.logger.info(`✅ [${listener.name}] Successfully parsed ${parsedCount} events`);
                            }
                        }
                        ...
                        if (listener.errorCount <= 3) {
                            logger_1.logger.error(`⚠️  Error fetching logs for ${listener.name}:`, logError);
                        }
                        // Stop if too many errors
                        if (listener.errorCount > 10) {
                            logger_1.logger.error(`❌ Too many errors fetching ${listener.name} logs`);
                ...
                listener.errorCount++;
                if (listener.errorCount <= 3) {
                    logger_1.logger.error(`⚠️  Error polling ${listener.name}:`, error);
                }
            }
        }, POLL_INTERVAL);
...
function handleEvent(contractName, eventName, args) {
    logger_1.logger.info(`Processing ${contractName}.${eventName}`, args);
    switch (eventName) {
        case 'DisputeCreated':
        ...
        case 'ProjectCreated':
            ...
            break;
        default:
            logger_1.logger.debug(`Unhandled event: ${eventName}`);
    }
}
function handleDisputeCreated(args) {
  ...
  `);
    try {
        (0, database_1.insertDispute)(projectId, projectId, milestoneId, initiator);
        logger_1.logger.info('Dispute saved to database', { projectId, milestoneId, initiator });
    }
    catch (error) {
        logger_1.logger.error('Failed to save dispute', error);
    }
    handleDisputeWithAI(projectId, milestoneId, initiator);
}
function handleProjectCreated(args) {
    const projectId = args[0];
    const client = args[1];
  ...
  `);
    try {
        (0, database_1.getOrCreateUser)(client);
        (0, database_1.getOrCreateUser)(freelancer);
        logger_1.logger.info('Users created', { client, freelancer });
    }
    catch (error) {
        logger_1.logger.error('Failed to create users', error);
    }
}
async function handleDisputeWithAI(projectId, milestoneId, initiatorAddress) {
    console.log(`
  ...
  `);
    try {
        const analysis = await (0, ai_engine_1.analyzeDispute)(projectId, milestoneId, initiatorAddress, 'Dispute raised for evaluation');
        (0, database_1.insertAIAnalysis)(projectId, analysis.complianceScore, analysis.qualityScore, analysis.timelineScore, analysis.verdict, analysis.confidence, analysis.reasoning);
        logger_1.logger.info('AI analysis completed', analysis);
        console.log(`
...
startListeners().catch((error) => {
    logger_1.logger.error('❌ Error starting listeners:', error);
    process.exit(1);
});
// Start API server
(0, api_1.startAPI)();
